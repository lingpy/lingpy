...
@cython.locals( i=cython.int, j=cython.int, almA=cython.list, almB=cython.list,
        matrix=cython.list, traceback=cython.list, gapA=cython.float,
        gapB=cython.float, match=cython.float, sim=cython.float)
cpdef tuple globalign( list seqA, list seqB, list gopA, list gopB, str proA, str
        proB, int M, int N, float scale, float factor, object scorer)
